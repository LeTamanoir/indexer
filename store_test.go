...
func testChainStore(t *testing.T, store ChainStore) {
	...
	canonical, ok, err := store.CanonicalBlock(ctx, 1)
	if err != nil || !ok || canonical.Hash != block1.Hash {
		t.Fatalf("CanonicalBlock(1) = %+v, %v, %v; want block1, true, nil", canonical, ok, err)
	}
	...
	canonical, ok, err = store.CanonicalBlock(ctx, 2)
	if err != nil || !ok || canonical.Hash != block2b.Hash {
		t.Fatalf("CanonicalBlock(2) after reorg = %+v, %v, %v; want block2b, true, nil", canonical, ok, err)
	}
